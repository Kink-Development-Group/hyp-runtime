...
impl TypeChecker {
    ...
    /// Register builtin function signatures from the central registry.
    ///
    /// The signatures live in [`crate::builtin_registry::BUILTINS`], the
    /// single source of truth shared with the CLI documentation output.
    fn register_builtins(&mut self) {
        for sig in crate::builtin_registry::BUILTINS {
            self.register_builtin(
                sig.name,
                sig.params.iter().map(|ty| ty.to_hypno_type()).collect(),
                sig.returns.to_hypno_type(),
            );
        }
    }
